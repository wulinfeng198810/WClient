 ...
 */
typedef enum{
    LTConversationTypeChat  = 0,    /*! \~chinese 单聊会话 \~english Chat */
    LTConversationTypeGroupChat,    /*! \~chinese 群聊会话 \~english Group chat */
    LTConversationTypeChatRoom      /*! \~chinese 聊天室会话 \~english Chatroom chat */
} LTConversationType;
 ...
 *  Conversation type
 */
@property (nonatomic, assign, readonly) LTConversationType type;
 ...
 */
@property (nonatomic, strong, readonly) LTMessage *latestMessage;
 ...
 */
- (BOOL)insertMessage:(LTMessage *)aMessage;
 ...
 *  @result Message insert result, YES: success, No: fail
 */
- (BOOL)appendMessage:(LTMessage *)aMessage;
 ...
 */
- (BOOL)updateMessage:(LTMessage *)aMessage;
 ...
 */
- (LTMessage *)loadMessageWithId:(NSString *)aMessageId;
 ...
 *
 *  @result Message list<LTMessage>
 */
- (NSArray *)loadMoreMessagesFromId:(NSString *)aMessageId
                              limit:(int)aLimit
                          direction:(LTMessageSearchDirection)aDirection;
 ...
 *  @param aLimit       Count of messages to load
 *  @param aSender      Message sender, will ignore it if it's LTpty
 *  @param aDirection   Message search direction
 *
 *  @result Message list<LTMessage>
 */
- (NSArray *)loadMoreMessagesWithType:(LTMessageBodyType)aType
 ...
 *
 *  @param aKeywords    Search content, will ignore it if it's LTpty
 *  @param aTimestamp   Reference timestamp
 *  @param aLimit       Count of messages to load
 *  @param aSender      Message sender, will ignore it if it's LTpty
 *  @param aDirection    Message search direction
 ...
 *
 *  \~english
 *  Load messages from DB in duration, result messages are sorted by receive time, user should limit the max count to load to avoid mLTory issue
 *
 ...
 *  @result Message instance
 */
- (LTMessage *)latestMessageFromOthers;
