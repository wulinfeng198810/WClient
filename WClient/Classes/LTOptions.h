 ...
 */
typedef enum {
    LTLogLevelDebug = 0, /*! \~chinese 输出所有日志 \~english Output all log */
    LTLogLevelWarning,   /*! \~chinese 输出警告及错误 \~english Output warning and error */
    LTLogLevelError      /*! \~chinese 只输出错误 \~english Output error only */
} LTLogLevel;
...
/*!
 *  \~chinese
 *  日志输出级别, 默认为LTLogLevelDebug
 *
 *  \~english
 *  Log output level, default is LTLogLevelDebug
 */
@property (nonatomic, assign) LTLogLevel logLevel;
 ...
 *  是否使用开发环境, 默认为NO
 *
 *  只能在[LTClient initializeSDKWithOptions:]时设置，不能在程序运行过程中动态修改
 *
 *  \~english
 *  Whether using development environment, default is NO
 *
 *  Can only set when initialize sdk [LTClient initializeSDKWithOptions:], can't change it in runtime
 */
 ...
 *  是否自动登录, 默认为YES
 *
 *  设置的值会保存到本地。初始化LTOptions时，首先获取本地保存的值
 *
 *  \~english
 *  Whether auto login, default is YES
 *
 *  Value will be saved to the local. When initialization LTOptions, the first to get the value of the local saved
 */
